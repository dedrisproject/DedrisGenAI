...
Fooocus. The PHP web UI (``webui/``) is the only client; it proxies browser
requests here (so no CORS is required, though a permissive header is added
defensively).
...
* Startup mirrors ``launch.py`` *without* running pip: it sets the required env
  ...
  first generate, exactly as in Fooocus.
...
Endpoints (all JSON, see DEDRIS_SPEC.md §5):
    GET  /api/health
    GET  /api/options
...
def _style_preview_url(style_name):
    ...
    # ever drifts from the file name. The style-expansion symbol is being renamed
    # fooocus_expansion -> dedris_expansion_style; try the new name first and fall
    # back to the old so this works regardless of merge timing.
    expansion_name = getattr(sdxl_styles, "dedris_expansion_style",
                             getattr(sdxl_styles, "fooocus_expansion", None))
...
def build_async_task_args(body):
    ...
    # 8 seed (-1 / missing => random within Fooocus seed space)
    import random
