...
# NOTE: the heavy imports that this module used to do at load time
# (``rembg`` -> ``pymatting`` -> ``cupy``, plus GroundingDINO / SAM /
# segment_anything and their large models) are intentionally deferred into the
# functions that actually use them. This keeps the module importable with only
# light deps (sys, numpy, torch, modules.config) so that the engine
# (server.py -> modules.async_worker -> this module) can start on environments
# where the cupy chain is broken or unavailable (e.g. Google Colab, where the
# preinstalled cupy is ABI-incompatible with numpy). The actual mask work is
# only attempted at call time, where the import cost / availability matters.
...
def optimize_masks(masks: torch.Tensor) -> torch.Tensor:
    ...
    """
    # Imported lazily (see module docstring): segment_anything pulls heavy deps.
...
def generate_mask_from_image(image: np.ndarray, mask_model: str = 'sam', extras=None,
    ...
    # Heavy deps imported lazily (see module docstring) so that merely importing
    # this module does not pull the rembg -> pymatting -> cupy chain or the
    # GroundingDINO / SAM models. These are only needed when a mask is actually
    # generated, which never happens during plain text-to-image generation.
