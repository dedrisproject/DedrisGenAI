...
from modules.config import path_dedris_expansion
...
class DedrisExpansion:
    def __init__(self):
        self.tokenizer = AutoTokenizer.from_pretrained(path_dedris_expansion)

        positive_words = open(os.path.join(path_dedris_expansion, 'positive.txt'),
        ...
        print(f'DedrisGenAI V2 Expansion: Vocab with {len(debug_list)} words.')
        ...
        self.model = AutoModelForCausalLM.from_pretrained(path_dedris_expansion)
        self.model.eval()
        ...
        print(f'DedrisGenAI Expansion engine loaded for {load_device}, use_fp16 = {use_fp16}.')
    ...
    def __call__(self, prompt, seed):
        ...
        if self.patcher.current_device != self.patcher.load_device:
            print('DedrisGenAI Expansion loaded by itself.')
