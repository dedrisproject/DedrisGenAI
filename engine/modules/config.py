...
def try_load_deprecated_user_path_config():
    ...
    try:
        ...
        replace_config('fooocus_expansion_path', 'path_dedris_expansion')
...
path_dedris_expansion = get_dir_or_set_default('path_dedris_expansion', '../models/prompt_expansion/dedris_expansion')
...
default_temp_path = os.path.join(tempfile.gettempdir(), 'dedrisgenai')
...
default_metadata_scheme = get_config_item_or_set_default(
    ...
    default_value=MetadataScheme.DEDRIS.value,
...
def get_model_filenames(folder_paths, extensions=None, name_filter=None):
    if extensions is None:
        extensions = ['.pth', '.ckpt', '.bin', '.safetensors', '.dedris.patch', '.fooocus.patch']
    files = []
...
def downloading_inpaint_models(v):
    ...
    load_file_from_url(
        ...
        file_name='dedris_inpaint_head.pth'
    )
    head_file = os.path.join(path_inpaint, 'dedris_inpaint_head.pth')
    patch_file = None
    ...
    if v == 'v1':
        load_file_from_url(
            ...
            model_dir=path_inpaint,
            file_name='inpaint.dedris.patch'
        )
        patch_file = os.path.join(path_inpaint, 'inpaint.dedris.patch')

    if v == 'v2.5':
        load_file_from_url(
            url='https://huggingface.co/lllyasviel/fooocus_inpaint/resolve/main/inpaint_v25.fooocus.patch',
            model_dir=path_inpaint,
            file_name='inpaint_v25.dedris.patch'
        )
        patch_file = os.path.join(path_inpaint, 'inpaint_v25.dedris.patch')

    if v == 'v2.6':
        load_file_from_url(
            url='https://huggingface.co/lllyasviel/fooocus_inpaint/resolve/main/inpaint_v26.fooocus.patch',
            model_dir=path_inpaint,
            file_name='inpaint_v26.dedris.patch'
        )
        patch_file = os.path.join(path_inpaint, 'inpaint_v26.dedris.patch')
...
def downloading_controlnet_cpds():
    load_file_from_url(
        ...
        model_dir=path_controlnet,
        file_name='dedris_xl_cpds_128.safetensors'
    )
    return os.path.join(path_controlnet, 'dedris_xl_cpds_128.safetensors')
...
def downloading_ip_adapters(v):
    ...
    load_file_from_url(
        ...
        model_dir=path_controlnet,
        file_name='dedris_ip_negative.safetensors'
    )
    results += [os.path.join(path_controlnet, 'dedris_ip_negative.safetensors')]
...
def downloading_upscale_model():
    load_file_from_url(
        ...
        file_name='dedris_upscaler_s409985e5.bin'
    )
    return os.path.join(path_upscale_models, 'dedris_upscaler_s409985e5.bin')
