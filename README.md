...
The engine is built on a proven, high-performance image-generation pipeline, re-architected around a
local HTTP API and a modern PHP front end.
...
2. **Python engine (`engine/server.py`)** — an HTTP service wrapping the generation pipeline.
