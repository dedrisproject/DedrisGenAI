...
DedrisGenAI's generation engine is based on the upstream **Fooocus** project (see the project
README's Credits section). The entries below are the upstream engine's release history, preserved
here for attribution and reference. They describe the behavior DedrisGenAI inherits; version numbers
and links point at the upstream project. References to "Fooocus …" style names below are historical —
in DedrisGenAI these styles ship under the **DedrisGenAI** name (e.g. "Fooocus V2" → "DedrisGenAI V2").

The current DedrisGenAI engine version is in `engine/version.py`.
...
1. Open a terminal in the project root and run `git pull`.
2. Update packages — the simplest path is to delete `runtimes/python/` and re-run the launcher
   (`start.bat` on Windows or `start.command` on macOS) so the portable runtime is re-provisioned.
   To update manually, install `engine/requirements_versions.txt` into the portable Python runtime,
   or use the Docker image (see [docker.md](docker.md)).
